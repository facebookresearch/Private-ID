...
use common::timer;
use protocol::{
    dspmc::{helper::HelperDspmc, traits::HelperDspmcProtocol},
    shared::TFeatures,
};
use rpc::proto::{
    common::Payload,
    gen_dspmc_helper::{
        dspmc_helper_server::DspmcHelper, service_response::*,
        Commitment, CommitmentAck, EHelperAck, ServiceResponse,
        UPartnerAck, CompanyPublicKeyAck
    },
...
impl DspmcHelper for DspmcHelperService {
    ...
    async fn recv_helper_public_key(&self, _: Request<ServiceResponse>) -> Result<Response<Self::RecvHelperPublicKeyStream>, Status> {
        let _ = timer::Builder::new()
            .label("server")
    ...
    async fn send_encrypted_keys(
        ...
        let ct_offset_len = u64::from_le_bytes(
            data.pop().unwrap().buffer.as_slice().try_into().unwrap(),
        ) as usize;
        // flattened len
        let ct_data_len = u64::from_le_bytes(
            data.pop().unwrap().buffer.as_slice().try_into().unwrap(),
        ) as usize;
